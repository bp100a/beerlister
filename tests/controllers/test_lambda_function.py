...
class TestAWSlambda(TestwithMocking):
    ...
    def test_open_skill(self):
        ...
        response = lambda_function.lambda_handler(
            event=event["payload"]["content"]["invocationRequest"]["body"], context=None
        )
        assert response is not None
    ...
    def test_session_state(self):
        ...
        list_breweries_event = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "ListBreweries", "mocked": True},
            },
            "session": {"new": False},
        }
        response = lambda_function.lambda_handler(
            event=list_breweries_event, context=None
        )
        assert response["response"]["shouldEndSession"]
    ...
    def test_fallback_response(self):
        get_fallback = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "AMAZON.FallbackIntent", "mocked": True},
            },
            "session": {"new": False},
        }
        ...
        assert (
            response["response"]["outputSpeech"]["text"]
            == lambda_function.FALLBACK_MESSAGE
        )
    ...
    def test_cancel_response(self):
        get_cancel = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "AMAZON.CancelIntent", "mocked": True},
            },
            "session": {"new": False},
        }
        ...
        assert response["response"]["shouldEndSession"]
        assert (
            response["response"]["outputSpeech"]["text"] == lambda_function.STOP_MESSAGE
        )
    ...
    def test_stop_response(self):
        get_stop = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "AMAZON.StopIntent", "mocked": True},
            },
            "session": {"new": False},
        }
    ...
    def test_help_response(self):
        get_help = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "AMAZON.HelpIntent", "mocked": True},
            },
            "session": {"new": False},
        }
        ...
        assert not response["response"]["shouldEndSession"]
        assert (
            response["response"]["outputSpeech"]["text"] == lambda_function.HELP_MESSAGE
        )
    ...
    def test_unknown_intent_response(self):
        get_unknown = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "JerseyBeers.UNKNOWN_INTENT", "mocked": True},
            },
            "session": {"new": False},
        }
    ...
    def test_end_session(self):
        event_end_session = {
            "session": {"new": False},
            "request": {"type": "SessionEndRequest"},
        }
    ...
    def test_set_home_brewery(self):
        ...
        assert response is not None
        assert response["response"]["outputSpeech"]["text"].startswith(
            "Your home brewery has been set to"
        )
        assert response["response"]["shouldEndSession"]
    ...
    def test_get_tap_list_logging(self):
        ...
        response = lambda_function.lambda_handler(
            event=get_tap_list_event, context=None
        )
        assert response is not None
        assert response["response"]["outputSpeech"]["text"].startswith(
            "Here are the breweries I know:"
    ...
    def test_set_bad_home_brewery(self):
        """Test that we get back the brewery list for an unknown brewery"""
        fn: str = self.data_dir() + "SetHomeBreweryIntent_RinnDuin.json"
        fp = open(fn, mode="r", encoding="utf8")
        json_intent = fp.read()
        fp.close()
        event = json.loads(json_intent)
    ...
    def test_bad_home_brewery_taplist(self):
        ...
        response = lambda_function.lambda_handler(
            event=home_taplist_event, context=None
        )
        assert response is not None
        assert response["response"]["outputSpeech"]["text"].startswith(
            "Sorry, no home brewery has been set"
        )
        assert response["response"]["shouldEndSession"]
    ...
    def test_home_brewery_taplist(self):
        ...
        assert response is not None
        assert response["response"]["outputSpeech"]["ssml"].startswith(
            "<speak>on tap at"
        )
        assert response["response"]["shouldEndSession"]
    ...
    def test_set_home_brewery_no_slot(self):
        ...
        set_home_event = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "SetHomeBrewery", "mocked": True},
            },
            "session": {"new": False, "user": {"userId": "valid_user_id"}},
        }
        response = lambda_function.lambda_handler(event=set_home_event, context=None)
        ...
        assert (
            lambda_function.ERROR_NO_BREWERY
    ...
    def test_empty_brewery(self):
        ...
        response = lambda_function.lambda_handler(
            event=empty_brewery_event, context=None
        )
        assert response is not None
        assert response["response"]["outputSpeech"]["type"] == "PlainText"
        assert response["response"]["outputSpeech"]["text"].startswith(
    ...
    def test_get_taplist_no_slot(self):
        ...
        empty_brewery_event = {
            "request": {
                "type": "IntentRequest",
                "intent": {"name": "GetTapListIntent", "mocked": True},
            },
            "session": {"new": False, "user": {"userId": "bogus_user_id"}},
        }
        response = lambda_function.lambda_handler(
