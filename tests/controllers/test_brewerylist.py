...
class TestBreweryList(TestwithMocking):
    """everything we need to test our list of breweries"""

    def test_brewery_list(self):
        """Test that all know brewery objects are listed in the global variable brewery_page_list"""
        brewery_page_objects = [
            "<class 'models.breweries.custom.departedsoles.DepartedSolespage'>",
            ...
            "<class 'models.breweries.custom.kane.KanePage'>",
        ]
    ...
    def test_find_breweries(self):
        ...
        known_breweries: dict[str, str] = {
            "Twin Elephant": "<class 'models.breweries.custom.twinelephant.TEBpage'>",
        ...
        for brewery_alias in known_breweries:
            brewery_obj, brewery_id = brewerylist.BREWERY_PAGES.find_brewery(
                brewery_alias
            )
            if str(type(brewery_obj)) != known_breweries[brewery_alias]:
                self.assertEqual(str(type(brewery_obj)), known_breweries[brewery_alias])
            assert brewery_id is not None

    def test_no_brewery(self):
        """Test that an unknown brewery is probably flagged"""
        brewery_obj, brewery_id = brewerylist.BREWERY_PAGES.find_brewery(
            "no such brewery"
        )
        assert brewery_obj is None and not brewery_id
    ...
    def test_set_home_brewery(self):
        ...
        self.assertFalse(brewerylist.BREWERY_PAGES.get_home_brewery(user_id))

        # now set a "home" brewery
        assert brewerylist.BREWERY_PAGES.add_home_brewery(
            brewery_name=brewery, user_id=user_id
        )

        # now check we have a "home" brewery
        home_brewery = brewerylist.BREWERY_PAGES.get_home_brewery(user_id).decode(
            "utf-8"
        )
    ...
    def test_set_bad_home_brewery(self):
        ...
        assert not brewerylist.BREWERY_PAGES.add_home_brewery(
            brewery_name=brewery, user_id=user_id
        )
