...
class UnTappdPage(BreweryPage):
    ...
    def __init__(self, **kwargs):
        ...
        self._alias = {
            ...
            "Pinelands": ["Pinelands Brewing", "Pinelands Brewery"],
            "Untied": [
                "Untied Brewing",
                "Untied Brewery",
                "United",
    ...
    def parse_inner_content(self, beer) -> None:
        ...
        for content in beer:
            if (
                hasattr(content, "attrs")
                and "class" in content.attrs
                and "beer-details" in content.attrs["class"]
            ):
                for i_c in content:
        ...
        for beer_entry in all_list:
            if "menu-info" in beer_entry.attrs["class"]:
                ...
                if len(beer_entry.contents[3].contents) > 2:
                    try:
                        beer_name = (
                            beer_entry.contents[1].contents[1].text.split(".")[1]
                        )
                        beer_style = (
                            beer_entry.contents[3].contents[1].text.strip("\\n ")
                        )
    ...
    def fetch_taplist(self, **kwargs) -> bool:
        ...
        # construct our URL
        url = "https://business.untappd.com/locations/{0}/themes/{1}/js".format(
            BREWERY_INFO[brewery][0], BREWERY_INFO[brewery][1]
        )
